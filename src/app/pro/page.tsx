...
import { PRO_PLANS } from "../constans";    
...
const ProPage = () => {
  ...
  const triggerCheckoutSession = useAction(api.stripe.createProPlanCheckoutSession);
  ...
  const isYearlySubscriptionActive =
    ...
    subscriptionData?.planType === "year";
  ...
  const handlePlanSelection = async (planId: "month" | "year") => {
    ...
    try {
      const result = await triggerCheckoutSession({ planId });
  ...
  return (
    <div className="container mx-auto px-4 py-16 max-w-6xl min-h-full">
      ...
      {isUserLoaded && subscriptionData?.status === "active" && (
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-8 rounded-md">
          <p className="text-blue-700">
            You have an active{" "}
            <span className="font-semibold">{subscriptionData.planType}</span>{" "}
            subscription.
          </p>
        </div>
      )}
          ...
          >
            ...
            <CardFooter className="mt-auto">
              <Button
                ...
                disabled={
                  subscriptionData?.status === "active" &&
                  (subscriptionData.planType === plan.id ||
